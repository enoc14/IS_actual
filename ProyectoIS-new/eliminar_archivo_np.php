<?php
	...
	$page_title = 'Eliminar archivo no publicado';
	include 'includes/menu.php'; // incluir en diseño del menú a la página

	echo "<div class='cont'>
	<h1>Eliminar un archivo no publicado<h1>";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($_POST['sure'] == 'Yes') { // Es seguro eliminar el registro
			...
			$query = "call eliminarArchivo('$a')";		
			$resul = @mysqli_query ($conexion, $query);
			if (mysqli_affected_rows($conexion) == 1){
				// mostrar el mensaje
				echo '<script>alert("El archivo ha sido eliminado con éxito")</script>';				
				echo "<script>location.href='publicar.php'</script>";
			}
			else { // si la consulta no tuvo éxito
				echo '<p class="error">Por un error del sistema, el archivo NO fue eliminado</p>';
				echo '<p>' . mysqli_error($conexion) . '<br />Query: ' . $query . '</p>'; 
			}
		} else{
			echo '<script>alert("El archivo NO fue eliminado")</script>';				
			echo "<script>location.href='publicar.php'</script>";
		}
	}
		...
		if (mysqli_num_rows($resul) == 1) { // Se encontró el ID
			...
			// crear el formulario
			echo '<form action="eliminar_archivo_np.php" method="POST">
					<div>
						<p>
